...
public class UserRepository {
	...
	public List<UserEntity> getAllUsers() throws IOException, ClassNotFoundException {
		...
		/*
		 * File file = new File(fileName); if (file.exists() && file.isFile()) { try
		 * (ObjectInputStream input = new ObjectInputStream(new FileInputStream(file)))
		 * { while (true) { try { UserEntity user = (UserEntity) input.readObject();
		 * users.add(user); } catch (EOFException e) { break; // Reached end of file } }
		 * } } else { throw new IOException("Invalid file: " + fileName); }
		 */
		return users;
	}
		...
		if (file.exists()) {
			try (ObjectInputStream input1 = new ObjectInputStream(new FileInputStream(file))) {
				...
				if (user.getUserId().equals(userName) && user.getPassword().equals(password)) {
					...
					AuthenticationAspect authAspect = new AuthenticationAspect(user);
					return true;
				}
			} catch (EOFException e) {
				return false;
	...
	public void logout() {
		AuthenticationAspect authAspect = new AuthenticationAspect(new UserEntity());
	}
}
